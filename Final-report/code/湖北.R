...
fixed <- c(tau0 = log(delay1),tau1 = log(46), sigma = 1/4.77, gamma = 1/6.74)
...
plot(t1-33,R1,type='l',ylim=c(0,4),ylab='有效再生数',xlab='日期',xaxt="n")
axis(1,c(seq(0,150,30)),c("2020-01-10","2020-02-10","2020-03-10","2020-04-10","2020-05-10","2020-06-10"))

abline(h=1,v=log(R0)/k+11,lty=2)
text(T1+10,2,"2020-02-26")
title('湖北')


#新增
data2=as.data.frame(read.table("clipboard",header=T))
plot(data2$times+15,data2$new,type='l',lty=2,ylab='病例新增人数',xlab='日期',xaxt="n")
axis(1,c(seq(0,150,30)),c("2020-01","2020-02","2020-03","2020-04","2020-05","2020-06"))
points(data2$times+15,data2$actual,pch=20)
